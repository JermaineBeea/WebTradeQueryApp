...
public class TradeWebApplication {
    ...
    private boolean parseBooleanValue(String json, String key) {
        System.out.println("Parsing boolean from JSON: " + json);
        Pattern pattern = Pattern.compile("\"" + key + "\"\\s*:\\s*(true|false)");
        Matcher matcher = pattern.matcher(json);
        if (matcher.find()) {
            boolean result = "true".equals(matcher.group(1));
            System.out.println("Parsed " + key + " = " + result);
            return result;
        } else {
            System.out.println("Could not find " + key + " in JSON, defaulting to false");
            return false;
        }
    }
